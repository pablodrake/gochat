...
func (s *Server) Run() error {
    for {
        ...
        retry := askYesNo("Do you want to restart the server? [yes/no]: ", s.terminal)
        if !retry {
            return nil // Normal termination
        }
    }
...
func (s *Server) shutdown() {
    s.terminal.PrintMessage("Shutdown signal received. Initiating graceful shutdown...")
...
func main() {
    ...
    server := NewServer(200*time.Second, terminal)
